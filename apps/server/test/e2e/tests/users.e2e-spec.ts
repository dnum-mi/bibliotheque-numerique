...
describe('users (e2e)', () => {
  ...
  describe('GET and PATCH /users/me', () => {
            ...
            role: {
              ...
              options: {
                ...
                5: {
                  ...
                  demarche: {
                    id: 5,
                    title: '[UPDATE-IDENTIFICATION] Déclaration de FE',
                    types: ['ASSO'],
                    dsId: 4,
                  },
                },
    ...
    it('Should update user connected', async () => {
      const { id, firstname, job } = await dataSource.manager.findOne(User, {
        where: { email: emailInstructorConnected },
      })
      ...
      const userUpdated = await dataSource.manager.findOne(User, {
        where: { email: emailInstructorConnected },
      })
