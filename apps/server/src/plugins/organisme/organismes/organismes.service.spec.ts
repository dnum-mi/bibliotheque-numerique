...
describe('OrganismesService', () => {
  ...
  it('should create new Organisme', async () => {
    ...
    await service.upsertOrganisme(fakeOrgData[0].idRef, [fakeOrgData[0].organismesSource as unknown as string])
    ...
    expect(organismeFound).toHaveProperty('title', fakeOrgData[0].dataJson.titre)
    expect(organismeFound).toHaveProperty('zipCode', fakeOrgData[0].dataJson.adresse_siege.code_postal)
    expect(organismeFound).toHaveProperty('city', fakeOrgData[0].dataJson.adresse_siege.commune)
  })
  ...
  it('should return one message when a organisme is not found in API', async () => {
    ...
    await expect(
      service.upsertOrganisme(fakeOrgData.idRef, [fakeOrgData.organismesSource as unknown as string]),
