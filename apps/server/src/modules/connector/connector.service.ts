...
export class ConnectorService extends BaseEntityService<Connector> {
  ...
  private _upsertConnector (toUpsert: Partial<Connector>) {
    ...
    return this.repo
      ...
      .into(Connector)
      .values(toUpsert)
      .orUpdate(['name', 'method', 'url', 'params', 'query', 'typeAuth', 'token', 'updateAt'], 'UK_CONNECTOR_NAME', {
        skipUpdateIfNoValuesChanged: true,
      })
      .returning(['id', 'method', 'name', 'url', 'params', 'query', 'typeAuth', 'token'])
      .execute()
  }
  ...
  private buildUrl (url: string, params: Record<string, string>, query: Record<string, string>): string {
