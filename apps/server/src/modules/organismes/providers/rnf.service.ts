...
export class RnfService {
  ...
  async getFoundationsHistoryFromSiaf(
    idRnf: string,
  ): Promise<ISiafRnfHistoryOutput[]> {
    this.logger.verbose('getFoundationsHistory')
    if (!idRnf || !isRnfLuhnValid(idRnf)) {
      this.logger.error(
        `RNF ID is invalid: ${idRnf}${
          isRnfLuhnValid(idRnf) ? '' : ' (Luhn check failed)'
    ...
    const url = `${this.config.get('rnf.siafUrl')}/foundations/${idRnf}/history`
    ...
    return axios
      ...
      .catch((e) => {
        const code = e.response?.status
        if (code === 404) {
          this.logger.warn(`No history found for RNF ID: ${idRnf}`)
          return []
        }
        throw e
      })
