...
export class DossierSearchService extends BaseEntityService<Dossier> {
  ...
  private _buildMainCTE(demarcheId: number, fieldsId: string[]): string {
    ...
    .map((id: string) => {
      ...
      return `${this._buildArrayAggregateFromOneFieldId(id)} as "${id}"`
    })
    .filter((a) => !!a)
    .join(',')}
        FROM dossiers d
  ...
  async search(demarche: Partial<Demarche>, dto: SearchDossierDto): Promise<{ total: number; data: any[] }> {
    ...
    const query = `WITH
      ...
      ${this._buildMainCTE(demarche.id, dto.columns)}
